...
float blaupzt;
int farbcode = 0;
float hell_kalibrieren;
float Abweichung = 0.10;  // Für RGB%. Zu Beginn sind 10% Sollwertabweichung erlaubt
const float Abweichung_Helligkeit = 0.25;
float Abweichung_H = Abweichung_Helligkeit; // Für Helligkeit.
...
void setup()
{
  ...
  digitalWrite(s1, LOW); // Empfindlichkeit des Sensors auf 100%
  ...
  Drehschieber(5);
  Drehschieber(45); // Schieber leeren und unter den Sensor fahren
  for (i = 1 ; i < 21 ; i++)
  { 
    color(); 
    helligkeit = rot + gruen + blau; 
    Serial.print(F("Messung Nr.")); 
    Serial.print(i);
    Serial.print(F(": ")); 
    Serial.println(helligkeit); 
    hell_kalibrieren += helligkeit;
    lcd.setCursor(0, 0); 
    lcd.print(helligkeit);
  }

  hell_kalibrieren = hell_kalibrieren / 20; 
  delay(300);
  Serial.println(F("---------------"));
  Serial.print(F("   Mittelwert  ")); 
  Serial.println(hell_kalibrieren);
  Serial.print(F("-Tabellenwert  ")); 
  Serial.println(GWleer[3]);
  Serial.println("=====================");
  Serial.print(F("=   Differenz  ")); 
  x = hell_kalibrieren - GWleer[3]; 
  Serial.println(x); 
  Serial.println();
  lcd.setCursor(0, 1); 
  ...
  GWleer[3] = GWleer[3] + x;  
  GWblau[3] = GWblau[3] + x;   
  GWgruen[3] = GWgruen[3] + x;
  GWgelb[3] = GWgelb[3] + x;  
  GWpink[3] = GWpink[3] + x;   
  GWlila[3] = GWlila[3] + x;
  GWrot[3] = GWrot[3] + x;    
  GWorange[3] = GWbraun[3] + x;
...
{ 
  ...
  lcd.print(F("Scn#")); 
  lcd.print(loops); 
  lcd.print(F("   ")); // Durchläufe ausgeben
  ...
  TWblau[0] = (GWblau[0] * (1.0 - Abweichung));    
  TWblau[1] = (GWblau[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWblau[2] = (GWblau[1] * (1.0 - Abweichung));    
  TWblau[3] = (GWblau[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWblau[4] = (GWblau[2] * (1.0 - Abweichung));    
  TWblau[5] = (GWblau[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWblau[6] = (GWblau[3] * (1.0 - Abweichung_H));    
  TWblau[7] = (GWblau[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWgruen[0] = (GWgruen[0] * (1.0 - Abweichung));  
  TWgruen[1] = (GWgruen[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWgruen[2] = (GWgruen[1] * (1.0 - Abweichung));  
  TWgruen[3] = (GWgruen[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWgruen[4] = (GWgruen[2] * (1.0 - Abweichung));
  TWgruen[5] = (GWgruen[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWgruen[6] = (GWgruen[3] * (1.0 - Abweichung_H));  
  TWgruen[7] = (GWgruen[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWgelb[0] = (GWgelb[0] * (1.0 - Abweichung));    
  TWgelb[1] = (GWgelb[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWgelb[2] = (GWgelb[1] * (1.0 - Abweichung));    
  TWgelb[3] = (GWgelb[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWgelb[4] = (GWgelb[2] * (1.0 - Abweichung));    
  TWgelb[5] = (GWgelb[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWgelb[6] = (GWgelb[3] * (1.0 - Abweichung_H));    
  TWgelb[7] = (GWgelb[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWpink[0] = (GWpink[0] * (1.0 - Abweichung));    
  TWpink[1] = (GWpink[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWpink[2] = (GWpink[1] * (1.0 - Abweichung));    
  TWpink[3] = (GWpink[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWpink[4] = (GWpink[2] * (1.0 - Abweichung));    
  TWpink[5] = (GWpink[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWpink[6] = (GWpink[3] * (1.0 - Abweichung_H));    
  TWpink[7] = (GWpink[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWlila[0] = (GWlila[0] * (1.0 - Abweichung));    
  TWlila[1] = (GWlila[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWlila[2] = (GWlila[1] * (1.0 - Abweichung));    
  TWlila[3] = (GWlila[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWlila[4] = (GWlila[2] * (1.0 - Abweichung));    
  TWlila[5] = (GWlila[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWlila[6] = (GWlila[3] * (1.0 - Abweichung_H));    
  TWlila[7] = (GWlila[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWrot[0] = (GWrot[0] * (1.0 - Abweichung));      
  TWrot[1] = (GWrot[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWrot[2] = (GWrot[1] * (1.0 - Abweichung));      
  TWrot[3] = (GWrot[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWrot[4] = (GWrot[2] * (1.0 - Abweichung));      
  TWrot[5] = (GWrot[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWrot[6] = (GWrot[3] * (1.0 - Abweichung_H));      
  TWrot[7] = (GWrot[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWorange[0] = (GWorange[0] * (1.0 - Abweichung)); 
  TWorange[1] = (GWorange[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWorange[2] = (GWorange[1] * (1.0 - Abweichung)); 
  TWorange[3] = (GWorange[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWorange[4] = (GWorange[2] * (1.0 - Abweichung)); 
  TWorange[5] = (GWorange[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWorange[6] = (GWorange[3] * (1.0 - Abweichung_H)); 
  TWorange[7] = (GWorange[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWbraun[0] = (GWbraun[0] * (1.0 - Abweichung));   
  TWbraun[1] = (GWbraun[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWbraun[2] = (GWbraun[1] * (1.0 - Abweichung));   
  TWbraun[3] = (GWbraun[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWbraun[4] = (GWbraun[2] * (1.0 - Abweichung));   
  TWbraun[5] = (GWbraun[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWbraun[6] = (GWbraun[3] * (1.0 - Abweichung_H));   
  TWbraun[7] = (GWbraun[3] * (1.0 + Abweichung_H)); // Grenzwerte für Helligkeit

  TWleer[0] = (GWleer[0] * (1.0 - Abweichung));      
  TWleer[1] = (GWleer[0] * (1.0 + Abweichung)); // Grenzwerte für R-Wert
  TWleer[2] = (GWleer[1] * (1.0 - Abweichung));      
  TWleer[3] = (GWleer[1] * (1.0 + Abweichung)); // Grenzwerte für G-Wert
  TWleer[4] = (GWleer[2] * (1.0 - Abweichung));      
  TWleer[5] = (GWleer[2] * (1.0 + Abweichung)); // Grenzwerte für B-Wert
  TWleer[6] = (GWleer[3] * (1.0 - Abweichung_H));      
  ...
  if (farbcode == 0) {
    Drehschieber(113);  // Nur neuen Smartie holen, wenn kein Problemsmartie im Schacht ist
    delay(100);
  }
  ...
  if (farbcode == 99) {
    Fehler++; lcd.setCursor(0, 0); lcd.print(Fehler); lcd.print(F("xERR! T="));
  ...
  if (leer == 5) {
    lcd.setCursor(0, 0); lcd.print(F("..going 2 sleep 1 Min"));
  ...
  {
    Rutsche(WINKEL[farbcode]);
    Drehschieber(5);
    Abweichung_H = Abweichung_Helligkeit;
    farbcode = 0;
    Fehler = 0;
    leer = 0;
  }
